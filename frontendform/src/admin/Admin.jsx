    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await axios.delete(`http://127.0.0.1:8000/api/registers/${id}`);
...
const downlodExcelPdf=()=>{
  ...
  data.forEach((item, index) => {
    doc.text(
      `${index + 1}. ${item.username} | ${item.email} | ${item.course_enroll_date} | ${item.course} | ${item.phone_number} | ${item.job} | ${item.country} | ${item.service}`,
      10,
      20 + index * 10
    );
  });
